  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-end">
        ...
        <button className="h-10 bg-ink text-white px-6 rounded-full text-xs uppercase tracking-widest font-medium hover:bg-black transition-colors">
          + Add Service
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services?.map((service) => (
          <div key={service.id} className="bg-white rounded-2xl border border-black/5 p-6 shadow-sm flex flex-col">
            ...
            <button className="w-full h-10 border border-black/10 rounded-xl text-xs uppercase tracking-widest font-medium text-ink hover:border-plum-500 hover:text-plum-600 transition-colors">
              Edit Service
            </button>
          </div>
        ))}
